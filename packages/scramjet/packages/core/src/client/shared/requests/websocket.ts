...
export default function (client: ScramjetClient, self: GlobalThis) {
	...
	const socketstreammap: WeakMap<object, FakeWebSocketStreamState> =
		new WeakMap();
	client.idl.operation("WebSocket", {
		construct(ctx) {
	...
	client.idl.attribute("WebSocket.prototype.binaryType", {
		get(ctx) {
	...
	client.idl.attribute("WebSocket.prototype.bufferedAmount", {
		get(ctx) {
	...
	client.idl.attribute("WebSocket.prototype.extensions", {
		get(ctx) {
			const ws = socketmap.get(ctx.this);
			if (!ws) return ctx.get();
	...
	client.idl.attribute("WebSocket.prototype.onclose", {
		get(ctx) {
			const ws = socketmap.get(ctx.this);
			if (!ws) return ctx.get();
	...
	client.idl.attribute("WebSocket.prototype.onerror", {
		get(ctx) {
			const ws = socketmap.get(ctx.this);
			if (!ws) return ctx.get();
	...
	client.idl.attribute("WebSocket.prototype.onmessage", {
		get(ctx) {
			const ws = socketmap.get(ctx.this);
			if (!ws) return ctx.get();
	...
	client.idl.attribute("WebSocket.prototype.onopen", {
		get(ctx) {
			const ws = socketmap.get(ctx.this);
			if (!ws) return ctx.get();
	...
	client.idl.attribute("WebSocket.prototype.url", {
		get(ctx) {
			const ws = socketmap.get(ctx.this);
			if (!ws) return ctx.get();
	...
	client.idl.attribute("WebSocket.prototype.protocol", {
		get(ctx) {
			const ws = socketmap.get(ctx.this);
			if (!ws) return ctx.get();
	...
	client.idl.attribute("WebSocket.prototype.readyState", {
		get(ctx) {
			const ws = socketmap.get(ctx.this);
			if (!ws) return ctx.get();
	...
	client.idl.operation("WebSocket.prototype.send", {
		apply(ctx) {
			const ws = socketmap.get(ctx.this);
			if (!ws) return;
	...
	client.idl.operation("WebSocket.prototype.close", {
		apply(ctx) {
			const ws = socketmap.get(ctx.this);
			if (!ws) return;
	...
	client.idl.operation("WebSocketStream", {
		construct(ctx) {
			const fakeWebSocket = {};
	...
	client.idl.attribute("WebSocketStream.prototype.closed", {
		get(ctx) {
	...
	client.idl.attribute("WebSocketStream.prototype.opened", {
		get(ctx) {
			const ws = socketstreammap.get(ctx.this);
	...
	client.idl.attribute("WebSocketStream.prototype.url", {
		get(ctx) {
			const ws = socketstreammap.get(ctx.this);
	...
	client.idl.operation("WebSocketStream.prototype.close", {
		apply(ctx) {
			const ws = socketstreammap.get(ctx.this);
			if (ctx.args[0]) {
